...
public class TestTowerObject : MonoBehaviour
{
    ...
    void OnMouseDrag()
    {
        ...
        Vector3Int tilePosition = TileMapManager.Instance.tileMap.WorldToCell(mousePosition);
        ...
        if (tilePosition != previousTilePosition)
        {
            ...
            {
                TileMapManager.Instance.SetTileColor(tilePosition, new Color(0, 1, 0, 0.5f)); // 초록색 반투명
            }
            else
            {
                TileMapManager.Instance.SetTileColor(tilePosition, new Color(1, 0, 0, 0.5f)); // 빨간색 반투명
            }
