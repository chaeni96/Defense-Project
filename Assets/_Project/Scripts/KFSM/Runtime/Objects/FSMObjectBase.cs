    ...
    public abstract class FSMObjectBase : MonoBehaviour, IFSMSubscriber, IDependencyObject
    {
        ...
        public virtual void SetTrigger(Trigger trigger)
        {
            if (animator != null && trigger != Trigger.None)
            {
                ...
                if (!triggerExists)
                {
                    // µð¹ö±× ·Î±×¸¸ Ãâ·ÂÇÏ°í Æ®¸®°Å ½ÇÇàÇÏÁö ¾ÊÀ½
                    Debug.Log($"¾Ö´Ï¸ÞÀÌÅÍ¿¡ '{triggerName}' Æ®¸®°Å°¡ ¾ø½À´Ï´Ù. ¾Ö´Ï¸ÞÀÌ¼Ç º¯°æÇÏÁö ¾ÊÀ½.");
                    return;
                }
                ...
                {
                    // °°Àº ¾Ö´Ï¸ÞÀÌ¼Ç Æ®¸®°Å¸é ´Ù½Ã ½ÇÇàÇÏÁö ¾ÊÀ½
                    Debug.Log($"ÇöÀç '{triggerName}' ¾Ö´Ï¸ÞÀÌ¼ÇÀÌ ÀÌ¹Ì ½ÇÇà ÁßÀÔ´Ï´Ù. Æ®¸®°Å ½ºÅµ.");
                    return;
                }
                ...
                {
                    Debug.Log($"¾Ö´Ï¸ÞÀÌ¼Ç Æ®·£Áö¼Ç ½ºÅµ: {triggerName}");
                ...
                currentAnimTrigger = triggerName; // ÇöÀç Æ®¸®°Å ¾÷µ¥ÀÌÆ®
                Debug.Log($"¾Ö´Ï¸ÞÀÌ¼Ç Æ®¸®°Å ¼³Á¤: {triggerName}");
            }
        }
